...
Instead of walking through scaffolding a new project, we can checkout a skeleton project from the book's github repo, which already has the necessary dependencies configured in `project.clj` as well as some basic markup and styles. See _Appendix B_ for information on checking out the correct version of code for this lesson. We will be working from tag `l10.1`.
